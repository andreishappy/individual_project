...
class XML_to_DSM:
    ...
    ##INSERT THE TOPOLOGY INTO THE NODES
        ...
        #Split them and start a thread for every 3 inserts
        inserts_per_thread = 1
        thread_specific_inserts = []
        number_of_threads = len(self.topology)/inserts_per_thread
        
        #Informs the simulator when the inserts are done
        inserter_threads = []
        for i in range(0,number_of_threads):
            start = i*inserts_per_thread
            if i != number_of_threads-1:
                end = start+inserts_per_thread
            else:
                end = len(self.topology)
            
            print "Starting InserterThread with links {0}".format(self.topology[start:end])
            inserter_thread = InserterThread(self.topology[start:end])
            inserter_thread.start()
            inserter_threads.append(inserter_thread)

        for thread in inserter_threads:
            thread.join()
    ...
    def run_simulation(self):
        ...
        while not self.monitor.all_threads_started():
            pass
        thread_start_time = time.time() - thread_start_time
        ...
        #Setting Topology
            ...
            self.set_topology()
            topology_start_time = time.time() - topology_start_time
            for thread in host_controllers:
                thread.stop()

            print "Thread start time {0}".format(thread_start_time)
            print "Topology setup time {0}".format(topology_start_time)
